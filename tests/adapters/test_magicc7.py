...
class TestMAGICC7(_AdapterTester):
    ...
    @mock.patch("pymagicc.core.config", {"executable_7": None})
    def test_no_magicc7(self):
        error_msg = re.escape("Could not find MAGICC7 executable: None")
        with pytest.raises(FileNotFoundError, match=error_msg):
            self.tadapter(ParameterSet(), ParameterSet())
    ...
    def test_initialize(self, test_adapter):
        ...
        assert (
            test_adapter._parameters.scalar(
                ("MAGICC7", "core_climatesensitivity"), "delta_degC"
            ).value
            == 3.0
        )
        assert (
            test_adapter._parameters.scalar(
                ("MAGICC7", "core_delq2xco2"), "W/m^2"
            ).value
            == 3.71
        )
